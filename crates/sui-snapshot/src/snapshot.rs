...
impl BucketWriter {
    ...
    fn write_object_blob(&mut self, object: &ObjectBlob) -> Result<()> {
        let mut buf = [0u8; MAX_VARINT_LENGTH];
        let n = (object.data.len() as u64).encode_var(&mut buf);
        self.write(&buf[0..n])?;
        buf[0] = object.encoding.into();
        self.write(&buf[0..OBJECT_ENCODING_BYTES])?;
        self.write(&object.data)?;
        Ok(())
    }
    fn write_object(&mut self, object: &Object) -> Result<()> {
        let blob = ObjectBlob::encode(object, Encoding::Bcs)?;
...
impl StateSnapshotWriterV1 {
        ...
        F: Fn(&ObjectRef) -> u32,
    {
        ...
        for object_ref in objects {
            ...
            bucket_writer.write_object(&object)?;
            counter += 1;
        }
        Ok(counter)
    }
...
impl StateSnapshotReaderV1 {
    fn new(dir_path: &Path) -> Result<Self> {
        ...
        let mut offset = MANIFEST_FILE_HEADER_BYTES;
        while offset < (manifest_file_size - SHA3_BYTES) {
            let file_metadata = Self::parse_file_metadata(&mut manifest_reader)?;
        ...
        // Verify objects in object files have the same object refs as entries in ref file
        for (bucket_num, _) in ref_files.iter() {
            let obj_files = object_files.get(bucket_num).ok_or(anyhow!(
                "Failed to find object files for bucket: {bucket_num}"
            ))?;
            let ref_file = ref_files.get(bucket_num).ok_or(anyhow!(
                "Failed to find sha3 digest of ref for bucket: {bucket_num}"
            ))?;
            if Self::compute_sha3_digest_from_ref_file(ref_file, dir_path)?
                != Self::compute_sha3_digest_from_object_files(obj_files, dir_path)?
            {
                return Err(anyhow!(
                    "Reference file doesn't match object files for bucket: {}",
                    &bucket_num
                ));
            }
        }
    ...
    fn parse_file_metadata(reader: &mut BufReader<File>) -> Result<FileMetadata> {
    ...
    fn compute_sha3_digest_from_object_files(
        obj_files: &BTreeMap<u32, FileMetadata>,
        dir_path: &Path,
    ) -> Result<[u8; SHA3_BYTES]> {
        let mut hasher = Sha3_256::default();
        for metadata in obj_files.values() {
            let file_path = metadata.file_path(dir_path);
            let f = metadata.compression_type.stream_decompress(&file_path)?;
            let obj_iter = ObjectIter::new(f)?;
            for object in obj_iter {
                let reference = object.compute_object_reference();
                let buf = BucketWriter::serialize_object_ref(&reference)?;
                hasher.update(buf);
            }
        }
        Ok(hasher.finalize().digest)
    }
    fn compute_sha3_digest_from_ref_file(
        ref_file: &FileMetadata,
        dir_path: &Path,
    ) -> Result<[u8; SHA3_BYTES]> {
        let mut hasher = Sha3_256::default();
        let file_path = ref_file.file_path(dir_path);
        let f = ref_file.compression_type.stream_decompress(&file_path)?;
        let obj_ref_iter = ObjectRefIter::new(f)?;
        for object_ref in obj_ref_iter {
            let buf = BucketWriter::serialize_object_ref(&object_ref)?;
            hasher.update(buf);
        }
        Ok(hasher.finalize().digest)
    }
    ...
    pub fn ref_iter(&mut self, bucket_num: u32) -> Result<impl Iterator<Item = ObjectRef>> {
        ...
        ObjectRefIter::new(
            file_metadata
                .compression_type
                .stream_decompress(&file_metadata.file_path(&self.dir_path))?,
        )
    }
    pub fn obj_iter(
        &mut self,
        bucket_num: u32,
        part_num: u32,
    ) -> Result<impl Iterator<Item = Object>> {
        let file_metadata = self
            .object_files
            .get(&bucket_num)
...
impl ObjectRefIter {
    pub fn new(mut reader: Box<dyn Read>) -> Result<Self> {
...
impl ObjectIter {
    ...
    fn next_object(&mut self) -> Result<Object> {
