...
pub async fn check_gas_input(
    store: &AuthorityStore,
    epoch_store: &AuthorityPerEpochStore,
    transaction: &TransactionData,
) -> SuiResult<()> {
    // system transactions do not check for gas input
    if transaction.kind.is_system_tx() {
        return Ok(());
    }
    // TODO: this is guaranteed in validation of transaction on certificate creation,
    //       do we need it here?
    if transaction.gas_coins().is_empty() {
        return Err(SuiError::MissingGasPayment);
    }
    ...
    // verify and fetch gas coins
    let mut gas_objs = vec![];
    for obj_ref in transaction.gas_coins() {
        let obj = store.get_object_by_key(&obj_ref.0, obj_ref.1)?;
        let obj = obj.ok_or(SuiError::into_transaction_input_error(SuiError::ObjectNotFound {
            object_id: obj_ref.0,
