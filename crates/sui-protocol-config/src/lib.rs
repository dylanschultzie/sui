    ...
    fn get_for_version_impl(version: ProtocolVersion) -> Self {
        ...
        match version.0 {
            1 => Self {
                ...
                max_type_arguments: Some(128),
                max_type_argument_depth: Some(16),
                // TODO: does this need to be big? Will we have cause for big blobs
                // as args? This is the main driver of max possible input tx size
                // at the moment, so making it smaller would reduce input tx size range
                // significantly.
                // Alternatively: could remove limits on pure argument size and number
                // of publishes in a programmable tx, and replace with overall serialized
                // size limits.
                max_pure_argument_size: Some(100 * 1024),
